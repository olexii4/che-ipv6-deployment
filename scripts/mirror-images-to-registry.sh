...
#   --mode <minimal|full|devfile-server>  Images set to mirror:
...
#                             - devfile-server: Only python:3.11-alpine for test-ipv6-validation.sh (quick)
#   --mirror-airgap-samples              Mirror images from air-gap samples (default in --mode full)
#   --no-mirror-airgap-samples           Skip air-gap samples (e.g. in full mode)
...
#   --use-local             Use locally built dashboard/server images (containers-storage) instead of pulling from Quay.
#                           Use when you built images with podman but cannot push to Quay (e.g. quay.io/eclipse/che-dashboard:CHE-23674).
...
MIRROR_AIRGAP_SAMPLES=""  # "true", "false", or unset (default from mode)
...
# Large images (che-plugin-registry, universal-developer-image, che-code) need 25+ min over proxy.
SKOPEO_TIMEOUT_SECONDS="${SKOPEO_TIMEOUT_SECONDS:-1800}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --dry-run)
            ...
            shift
            ;;
        --mirror-airgap-samples)
            MIRROR_AIRGAP_SAMPLES=true
            shift
            ;;
        --no-mirror-airgap-samples)
            MIRROR_AIRGAP_SAMPLES=false
            shift
            ;;
        --help)
            grep '^#' "$0" | sed 's/^# //' | sed 's/^#//'
...
# Default air-gap samples mirroring from mode (full = true) unless explicitly set
if [ "${MIRROR_AIRGAP_SAMPLES}" != "true" ] && [ "${MIRROR_AIRGAP_SAMPLES}" != "false" ]; then
  [ "${MODE}" = "full" ] && MIRROR_AIRGAP_SAMPLES=true || MIRROR_AIRGAP_SAMPLES=false
fi

# Validate mode
if [ "${MODE}" != "minimal" ] && [ "${MODE}" != "full" ] && [ "${MODE}" != "devfile-server" ]; then
    echo -e "${RED}Error: Invalid --mode '${MODE}'. Expected 'minimal', 'full', or 'devfile-server'.${NC}"
    exit 1
fi
...
if [ "${PREFETCH_ONLY}" != "true" ]; then
    ...
    # Tries: ImageContentSourcePolicy -> ImageTagMirrorSet -> ImageDigestMirrorSet -> OpenShift route -> Metal³ virthost heuristic
    if [ -z "$LOCAL_REGISTRY" ]; then
        ...
        if [ -z "$LOCAL_REGISTRY" ]; then
            LOCAL_REGISTRY=$(oc get imagetagmirrorset -o jsonpath='{.items[0].spec.imageTagMirrors[0].mirrors[0]}' 2>/dev/null | sed 's|/.*||' || echo "")
        fi
        if [ -z "$LOCAL_REGISTRY" ]; then
            LOCAL_REGISTRY=$(oc get imagedigestmirrorset -o jsonpath='{.items[0].spec.imageDigestMirrors[0].mirrors[0]}' 2>/dev/null | sed 's|/.*||' || echo "")
        fi
        if [ -z "$LOCAL_REGISTRY" ]; then
            LOCAL_REGISTRY=$(oc get route default-route -n openshift-image-registry -o jsonpath='{.spec.host}' 2>/dev/null || echo "")
        fi
        if [ -z "$LOCAL_REGISTRY" ]; then
            # Metal³/ostest: virthost.<basedomain>:5000 (registry runs on provisioning host)
            BASE_DOMAIN=$(oc get infrastructure cluster -o jsonpath='{.status.apiServerURL}' 2>/dev/null | sed -n 's|https://api\.\([^:/]*\).*|\1|p' || true)
            if [ -n "$BASE_DOMAIN" ]; then
                LOCAL_REGISTRY="virthost.${BASE_DOMAIN}:5000"
            fi
        fi

        if [ -z "$LOCAL_REGISTRY" ]; then
            echo -e "${RED}Error: Could not auto-detect local registry${NC}"
            echo "Please specify with: --registry <host:port>"
            exit 1
        fi
...
# Devfile-server only: quick mirror for test-ipv6-validation.sh
if [ "${MODE}" = "devfile-server" ]; then
  IMAGES+=("docker.io/library/python:3.11-alpine")
elif [ "${MODE}" = "minimal" ] || [ "${MODE}" = "full" ]; then
...
if [ "${MODE}" = "full" ]; then
  IMAGES+=(
    ...
    # Devfile server (test-ipv6-validation.sh) - serves devfiles over IPv6
    "docker.io/library/python:3.11-alpine"

    # Air-gap samples (manifests/che/air-gap-samples.json): php, nodejs-express, python
    # All use registry.redhat.io/devspaces/udi-rhel9:3.22
    "registry.redhat.io/devspaces/udi-rhel9:3.22"

    # che-gateway-patcher CronJob (manifests/che/che-gateway-patcher-cronjob.yaml)
...
# Discover images from cluster events (Failed/ErrImagePull with manifest unknown)
# Catches project-clone:sha-*, devworkspace-controller:sha-* etc. when DWO version changes
if [ "${PREFETCH_ONLY}" != "true" ] && [ -n "${KUBECONFIG}" ] && [ -f "${KUBECONFIG:-}" ]; then
  FAILED_IMAGES=$(oc get events -A --field-selector reason=Failed -o json 2>/dev/null | \
    python3 -c "
import json, sys, re
...
for item in data.get('items', []):
    ...
    if 'manifest unknown' in msg or 'Mirrors also failed' in msg:
        m = re.search(r'Failed to pull image \"([^\"]+)\"', msg)
        if m:
            img = m.group(1)
            if img not in seen and ('quay.io' in img or 'ghcr.io' in img or 'registry.' in img):
                seen.add(img)
                print(img)
" 2>/dev/null)
  if [ -n "${FAILED_IMAGES}" ]; then
    echo -e "${YELLOW}Discovering images from Failed events (manifest unknown):${NC}"
    while IFS= read -r img; do
      [ -z "${img}" ] && continue
      if echo "${img}" | grep -q "^${LOCAL_REGISTRY}/"; then continue; fi
      # Avoid duplicates
      if printf '%s\n' "${IMAGES[@]}" | grep -Fxq "${img}" 2>/dev/null; then continue; fi
      IMAGES+=("${img}")
      echo "  + ${img}"
    done <<< "${FAILED_IMAGES}"
    echo ""
...
# Mirror images from air-gap samples (manifests/che/air-gap-samples.json)
# Extracts container images from devfiles in build/air-gap; run prepare-airgap-from-samples.sh first if missing.
if [ "${MIRROR_AIRGAP_SAMPLES}" = "true" ]; then
  EXTRACT_SCRIPT="${SCRIPT_DIR}/extract-airgap-sample-images.sh"
  if [ -x "${EXTRACT_SCRIPT}" ]; then
    echo -e "${YELLOW}Extracting images from air-gap samples...${NC}"
    while IFS= read -r img; do
      [ -z "${img}" ] && continue
      if echo "${img}" | grep -q "^${LOCAL_REGISTRY}/"; then
        continue
      ...
      IMAGES+=("${img}")
    done < <("${EXTRACT_SCRIPT}" -p 2>/dev/null || "${EXTRACT_SCRIPT}" 2>/dev/null)
    echo ""
  else
    echo -e "${YELLOW}⚠ extract-airgap-sample-images.sh not found or not executable. Skipping air-gap samples.${NC}"
    echo ""
  fi
fi

# Mirror images from test-infrastructure devfiles (Node.js, Python for IPv6 devfile server testing)
# Devfiles in manifests/test-infrastructure/*-devfile.yaml and devfile-configmaps.yaml
TEST_INFRA_DIR="${REPO_DIR}/manifests/test-infrastructure"
if [ -d "${TEST_INFRA_DIR}" ]; then
  echo -e "${YELLOW}Extracting images from test-infrastructure devfiles...${NC}"
  while IFS= read -r img; do
    [ -z "${img}" ] && continue
    if [ -n "${LOCAL_REGISTRY}" ] && echo "${img}" | grep -q "^${LOCAL_REGISTRY}/"; then
      continue
    fi
    IMAGES+=("${img}")
  done < <(
    for f in "${TEST_INFRA_DIR}"/*-devfile.yaml "${TEST_INFRA_DIR}/devfile-configmaps.yaml"; do
      [ -f "$f" ] || continue
      grep -E '^\s+image:\s+' "$f" 2>/dev/null | sed -E 's/^[[:space:]]*image:[[:space:]]*//' | sed 's/[[:space:]]*#.*//' || true
    done | sort -u | grep -E '^(quay\.io|ghcr\.io|registry\.redhat\.io|registry\.access\.redhat\.com|docker\.io)/' || true
  )
  echo ""
fi
...
mirror_one_image() {
    ...
    if [ "${USE_LOCAL_IMAGES}" = "true" ]; then
        if [ "${source_image}" = "${DASHBOARD_IMAGE}" ] || [ "${source_image}" = "${CHE_SERVER_IMAGE}" ]; then
            # On macOS, skopeo and podman use different storage. Use podman save + docker-archive.
            LOCAL_TAR="${CACHE_DIR}/local_$(echo "${source_image}" | tr '/:' '__')_.tar"
            if command -v podman &>/dev/null && podman image exists "${source_image}" 2>/dev/null; then
                echo -e "${YELLOW}  Exporting local image with podman save...${NC}"
                if podman save -o "${LOCAL_TAR}" "${source_image}" 2>/dev/null; then
                    SOURCE_TRANSPORT="docker-archive:${LOCAL_TAR}"
                    echo -e "${YELLOW}  Using local image (docker-archive)${NC}"
                else
                    echo -e "${YELLOW}  ⚠ podman save failed, falling back to remote pull${NC}"
                fi
            elif skopeo inspect "containers-storage:${source_image}" &>/dev/null; then
                SOURCE_TRANSPORT="containers-storage:${source_image}"
                echo -e "${YELLOW}  Using local image (containers-storage)${NC}"
            else
                echo -e "${YELLOW}  ⚠ Local image not found, falling back to remote pull${NC}"
            fi
        fi
    fi
    if [ "${SOURCE_TRANSPORT}" = "docker://${source_image}" ] && [ "${USE_CACHE}" = "true" ]; then
    ...
    if [ "${MIRROR_OK}" = "true" ]; then
        ...
        echo "fail" > "${status_file}" 2>/dev/null || true
    fi
    echo ""
}
...
# Note: Do NOT trap EXIT to remove STATUS_DIR - background workers may still be
# writing when the main script exits (e.g. on signal). Clean up explicitly at end.
...
# Clean up status dir (workers have finished; safe to remove)
